...
class UsuarioService {
    static async criarUsuario(dados) {
        const { email, senha_hash, senha, id_perfil } = dados;
        const senhaInformada = senha_hash ?? senha;
        const perfil = id_perfil ?? 2;

        if (!email || !senhaInformada || perfil === undefined || perfil === null) {
        ...
        const resultado = await pool.query(
            `INSERT INTO usuarios (email, senha_hash, id_perfil, ativo, data_criacao)
             VALUES ($1, $2, $3, $4, $5)
             RETURNING *`,
            [email, senhaHash, perfil, true, new Date()]
        );
