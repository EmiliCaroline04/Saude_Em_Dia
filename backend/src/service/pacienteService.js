...
class PacienteService {
    static async criarPaciente(dados) {
        ...
        if (id_usuario) {
            ...
            const UsuarioService = require('./usuarioService');
            const emailFallback = `paciente-${cpf.replace(/\D/g, '')}@saudedia.local`;
            usuarioCriado = await UsuarioService.criarUsuario({
                email: emailFallback,
                senha_hash: senhaInformada || '123456',
                id_perfil: 3,
            });
        }
        ...
        const result = await pool.query(
            ...
            (id_usuario, nome, cpf, endereco, telefone, sexo, data_nascimento, tipo_sanguineo)
            VALUES ($1,$2,$3,$4,$5,$6,$7,$8)
            RETURNING *`,
            [usuarioCriado.id_usuario, nome, cpf, endereco, telefone, sexo, data_nascimento, tipo_sanguineo]
        );
